...
Appkit will own a protocol-neutral standalone compose surface: the editable-body boundary, one or more ordered parts, generated read-only context and status presentation, status-field actions, media-attachment presentation, point and modification invariants, an in-flight submit session with optional 0-1 progress and a cancel gesture, and reusable completion/media/UI primitives. Generated chrome is not buffer text: context, status, attachments, and footer are overlays, so they cannot enter undo or shift body positions. Buffer text is only the editable bodies plus a fixed read-only divider between parts. Clients decide whether those parts become one X self-thread, a Misskey reply chain, or a single note. The planned surface is `appkit-compose`, not an X, Mastodon, or Misskey model and not a forced specialization of `appkit-chatbuf`, which is designed around persistent chat/timeline input.
